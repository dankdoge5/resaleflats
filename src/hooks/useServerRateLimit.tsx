...
export const useServerRateLimit = () => {
  const checkRateLimit = useCallback(
    async (identifier: string, actionType: string, config: ServerRateLimitConfig): Promise<RateLimitResult> => {
      try {
        ...
        return { allowed: result.allowed, remainingAttempts: result.remaining_attempts, resetTime: result.reset_time };
      } catch (error) {

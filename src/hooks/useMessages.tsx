...
export const useMessages = () => {
  ...
  const fetchThreads = async () => {
    ...
    try {
      const { data: threadData, error: threadError } = await db
        ...
        .select(`*, thread_participants!inner(user_id)`)
      ...
      toast({ variant: 'destructive', title: 'Error', description: 'Failed to load message threads' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const createThread = async (title: string, participantIds: string[]) => {
    ...
    try {
      ...
      const { error: participantError } = await db
        .from('thread_participants')
        .insert(allParticipants.map((userId) => ({ thread_id: thread.id, user_id: userId })));
  ...
  const sendMessage = async (threadId: number, content: string) => {
    ...
    try {
      const { error } = await db.from('messages').insert({ thread_id: threadId, sender_id: user.id, content });
  ...
  return { threads, loading, createThread, sendMessage, fetchMessages, subscribeToThread, refreshThreads: fetchThreads };
};
